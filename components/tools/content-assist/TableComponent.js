    ...
    <table id="response-table">
      ...
      <tbody>
        {tableData.slice(1).map((rowData, rowIndex) => (
          <tr key={rowIndex} className={rowIndex % 2 === 0 ? 'respEven_row' : 'respOdd_row'}>
            <td className="table_cell cell_index_0">{rowIndex + 1}</td>
            {rowData.map((cellData, cellIndex) => (
              <td key={cellIndex} className={`table_cell cell_index_${cellIndex + 1}`}>
                {cellIndex === 0 ? (
                  <a className="website-link" href={cellData} target="_blank" rel="noopener noreferrer">
                    {cellData}
                  </a>
                ) : (
                  cellData
